...
describe('Cart', () => {
  ...
  it('should display all cart items', () => {
    const {fixture} = setup({cartItems: mockedGamesResponse});
    const items = fixture.debugElement.nativeElement.querySelectorAll(`[data-testid=${ElementTestId.CartItem}]`);
    expect(items.length).toBe(2);
  });

  it('should display the correct cart total', () => {
    const {fixture} = setup({cartItems: mockedGamesResponse});
    const total = getElementByTestId(fixture, ElementTestId.CartTotal)?.textContent?.trim();
    expect(total).toContain('12.40');
  });

  it('should show the correct number of items in the cart status', () => {
    const {fixture} = setup({cartItems: mockedGamesResponse});
    const status = getElementByTestId(fixture, ElementTestId.CartStatus)?.textContent?.trim();
    expect(status).toContain('2 Items in Cart');
  });

  it('should call clearCart when the clear cart button is clicked', () => {
    const clearItemsSpy = MockInstance(CartService, 'clearCart', jasmine.createSpy('clearCart'));
  ...
  it('should display correct image src and alt text for an item', () => {
    ...
    const images = fixture.debugElement.nativeElement.querySelectorAll(`[data-testid=${ElementTestId.CartItemImage}]`);
    expect(images[0].getAttribute('src')).toBe('gog-game-4.png');
    expect(images[0].getAttribute('alt')).toBe('Cover art of Neverwinter Nights');
  });
  ...
  it('should display zero price correctly if item price is 0', () => {
    ...
    const price = getElementByTestId(fixture, ElementTestId.CartItemPrice)?.textContent?.trim();
  ...
  it('should display the correct cart item names', () => {
    const {fixture} = setup({cartItems: mockedGamesResponse});
    const names = fixture.debugElement.nativeElement.querySelectorAll(`[data-testid=${ElementTestId.CartItemName}]`);
    expect(names.length).toBe(2);
    expect(names[0].textContent?.trim()).toBe('Neverwinter Nights');
    expect(names[1].textContent?.trim()).toBe("Assassin's Creed: Director's Cut");
  });

  it('should remove a single item when clicking its remove button', () => {
    const removeFromCartSpy = MockInstance(CartService, 'removeFromCart', jasmine.createSpy('removeFromCart'));
    const {fixture} = setup({cartItems: mockedGamesResponse});

    const removeButtons = fixture.debugElement.nativeElement.querySelectorAll(`[data-testid=${ElementTestId.RemoveItemFromCartButton}]`);
